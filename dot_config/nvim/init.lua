local function setup_packer(plugins, opts)
  opts = opts or {}
  local add_plugins = opts.add_plugins or {}
  local remove_plugins = opts.remove_plugins or {}
  ...
  require('packer').startup({
    function()
      for _, plugin in ipairs(plugins) do
        ...
        if remove_plugins[name] == nil then
          use(plugin)
        end
      end
    ...
    config = {
      profile = {
        enable = false, -- enable to have profiling
        threshold = 0,
      },
    },
  })
...
function MyTelescopeConfig(opts)
  opts = opts or {}
  ...
  return {
    {
      ...
      config = string.format('MyTelescopeConfig(%s)', vim.inspect(opts)),
    },
    {
...
local function get_lsp_plugins(opts)
  return {
    {
...
function MyStatuslineConfig(opts)
  opts = opts
    or {
...
local function get_statusline_plugins(opts)
  return {
    {
...
local function get_all_plugins(opts)
  local plugins = {}
  opts = opts or {}

  append_to_table(get_completion_plugins(), plugins)
  append_to_table(get_treesitter_plugins(), plugins)
  append_to_table(get_telescope_plugins(opts.telescope), plugins)
  append_to_table(get_statusline_plugins(opts.statusline), plugins)
  append_to_table(get_visual_tweak_plugins(), plugins)
  append_to_table(get_lsp_plugins(opts.lsp), plugins)
